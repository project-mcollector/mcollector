...
export default function LoginPage() {
  ...
  async function handlePasskeyLogin() {
    ...
    try {
      ...
      if (!optionsRes.ok) {
        setError(await readErrorMessage(optionsRes, t("passkeyOptionsError")));
        return;
      }
      ...
      if (!loginRes.ok) {
        setError(await readErrorMessage(loginRes, t("passkeyVerifyError")));
        return;
      }
      ...
      if (err instanceof DOMException && err.name === "SecurityError") {
        setError(t("passkeySecurityError"));
        return;
      }
        ...
        <form onSubmit={handleSubmit} className={styles.form}>
          ...
          {emailUnconfirmed && (
            <div className={styles.error}>
              {t("confirmEmailFirst")}{" "}
              {resendDone ? (
                <span style={{ color: "#16a34a" }}>{t("emailSent")}</span>
              ) : (
                <button
                  type="button"
                ...
                >
                  {resendLoading ? tCommon("sending") : t("resend")}
                </button>
              )}
            </div>
          )}
          ...
          <div>
            <div className={styles.labelRow}>
              ...
              <Link href="/forgot-password" className={styles.link} style={{ fontSize: 13 }}>{t("forgotPassword")}</Link>
            </div>
          ...
          >
            ...
            {passkeyLoading ? t("passkeyWaiting") : t("passkeyLogin")}
          </button>
