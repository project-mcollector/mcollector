...
export default function DashboardPage() {
  ...
  }, [projectId, days, refreshKey, router, t]);
  ...
  useEffect(() => {
    ...
    async function loadCounts() {
      ...
      try {
        ...
        if (!cancelled) setCountsError(t("countsError"));
      }
    }

    loadCounts();
    return () => { cancelled = true; };
  }, [projectId, days, refreshKey, eventCountsPeriod, periodOffset, router, t]);
  ...
  async function confirmRename() {
    ...
    try {
      ...
      setRenameError(err instanceof Error ? err.message : t("loadError"));
    } finally {
      setRenaming(false);
    }
  }
  ...
  async function confirmDelete() {
    ...
    try {
      ...
      setDeleteError(err instanceof Error ? err.message : t("loadError"));
    } finally {
      setDeleting(false);
    }
  }
  ...
  if (!overview)
    return (
      <div className={styles.page}>
        {nav}
        <p className={styles.emptyState}>{t("noOverview")}</p>
      </div>
    );
  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.header}>
        <div>
          <div className={styles.projectNameRow}>
            <h1 className={styles.title}>{project?.name ?? "Dashboard"}</h1>
            {project && (
              <button
          ...
          {lastUpdated && (
            <p className={styles.subtitle}>
              {t("updatedAt", { time: lastUpdated.toLocaleTimeString(locale, { hour: "2-digit", minute: "2-digit" }) })}
            </p>
          )}
        </div>
      ...
      <div className={styles.statsGrid}>
        ...
        <div className={styles.statCard}>
          <p className={styles.statLabel}>{t("users")}</p>
          <p className={styles.statValue}>{formatNumber(overview.uniqueUsers)}</p>
        </div>
        <div className={styles.statCard}>
          <p className={styles.statLabel}>{t("pageViews")}</p>
          <p className={styles.statValue}>{formatNumber(overview.pageViews)}</p>
        </div>
      </div>

      <div className={styles.chartSection}>
        <h2 className={styles.chartTitle}>{t("eventsByDay")}</h2>
        {eventsTimeseries.length === 0 ? (
          <p className={styles.chartEmpty}>{t("noEvents")}</p>
        ) : (
          <ResponsiveContainer width="100%" height={300}>
            <LineChart data={eventsTimeseries}>
              ...
              <Line type="monotone" dataKey="count" name={t("events")} stroke="#8884d8" dot={false} />
            </LineChart>
          </ResponsiveContainer>
        )}
      </div>
      ...
      <div className={styles.chartSection}>
        <div className={styles.chartHeader}>
          <h2 className={styles.chartTitle}>{t("topEvents")}</h2>
          <div className={styles.countsControls}>
            ...
            <div className={styles.dateRange}>
              {(["total", "day", "week", "month"] as const).map((p) => (
                ...
                >
                  {p === "total" ? t("periodTotal") : p === "day" ? t("periodDay") : p === "week" ? t("periodWeek") : t("periodMonth")}
                </button>
              ))}
            </div>
          </div>
        ...
        {countsError ? (
          ...
          <p className={styles.chartEmpty}>{t("noData")}</p>
        ) : (
          <table className={styles.table}>
            <thead>
              <tr>
                <th>{t("name")}</th>
                <th>{eventCountsPeriod === "total" ? t("total") : eventCountsPeriod === "day" ? t("perDay") : eventCountsPeriod === "week" ? t("perWeek") : t("perMonth")}</th>
                <th></th>
              </tr>
            </thead>
            <tbody>
                ...
                <Fragment key={event.name}>
                  <tr>
                    ...
                    <td>
                      ...
                      >
                        {selectedEvent === event.name ? t("hide") : t("chart")}
                      </button>
                    </td>
                  </tr>
      ...
      <div className={styles.chartSection}>
        <h2 className={styles.chartTitle}>{t("usersByDay")}</h2>
        {usersTimeseries.length === 0 ? (
          <p className={styles.chartEmpty}>{t("noDataPeriod")}</p>
        ) : (
          <ResponsiveContainer width="100%" height={300}>
            <LineChart data={usersTimeseries}>
              ...
              <YAxis allowDecimals={false} />
              <Tooltip formatter={tooltipFormatter} labelFormatter={tooltipLabelFormatter} />
              <Line type="monotone" dataKey="count" name={t("users")} stroke="#82ca9d" dot={false} />
            </LineChart>
          </ResponsiveContainer>
        )}
      </div>
      ...
      {renameOpen && (
        <div className={styles.modalOverlay} onClick={() => setRenameOpen(false)}>
          <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
            <h2 className={styles.modalTitle}>{t("renameTitle")}</h2>
            <label className={styles.label}>{t("newName")}</label>
            <input
              type="text"
              value={renameName}
      ...
      {deleteConfirm && (
        <ConfirmModal
          title={t("deleteTitle")}
          message={t("deleteMessage", { name: project?.name ?? "" })}
