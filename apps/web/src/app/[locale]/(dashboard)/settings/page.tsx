...
export default function SettingsPage() {
  ...
  const tLabels = useTranslations("passkeyLabels");
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        const res = await authFetch("/api/auth/passkey", router);
        const data = (await res.json()) as PasskeyInfo[];
        ...
        if (!cancelled) {
          ...
          setErrorMessage(
            err instanceof Error
              ? localizeErrorMessage(err.message, tErrors)
              : localizeErrorMessage("", tErrors),
          );
        }
      } finally {
  ...
  async function handleDeletePasskey(credentialId: string) {
    ...
    try {
      ...
      setStatus("error");
      setErrorMessage(
        err instanceof Error ? localizeErrorMessage(err.message, tErrors) : localizeErrorMessage("", tErrors),
      );
    } finally {
      setDeletingId(null);
    }
  ...
  async function handleCreatePasskey() {
    if (passkeys.length >= MAX_PASSKEYS) {
      setStatus("error");
      setErrorMessage(t("passkeyLimitError", { max: MAX_PASSKEYS }));
      return;
    }
    ...
    try {
      ...
      if (!credential) {
        setStatus("error");
        setErrorMessage(t("deviceNoCredentials"));
        return;
      }
      ...
      if (err instanceof DOMException && err.name === "NotAllowedError") {
        ...
        if (err instanceof DOMException && err.name === "InvalidStateError") {
          setErrorMessage(t("passkeyExists"));
        } else if (err instanceof DOMException && err.name === "SecurityError") {
          setErrorMessage(t("browserSecurityError"));
        } else {
          setErrorMessage(
            err instanceof Error ? localizeErrorMessage(err.message, tErrors) : localizeErrorMessage("", tErrors),
          );
    ...
    <div className="p-8 max-w-2xl">
      ...
      <section>
        ...
        <div className="rounded-lg bg-sidebar p-6">
          ...
          <p className="text-sm text-muted-foreground mb-4">{t("passkeyDescription")}</p>

          {loadingPasskeys ? (
            <p className="text-sm text-muted-foreground mb-4">{t("loadingPasskeys")}</p>
          ) : passkeys.length > 0 ? (
            <ul className="mb-4 divide-y divide-border -mx-6">
              {passkeys.map((passkey) => (
                <li key={passkey.credentialId} className="flex items-center justify-between gap-2 px-6 py-2.5 text-sm">
                  <div>
                    <span className="font-medium">{getPasskeyLabel(passkey.aaguid, passkey.transports, passkey.isBackupEligible, tLabels)}</span>
                    <span className="text-muted-foreground"> — {t("createdAt", { date: new Date(passkey.createdAt).toLocaleString(locale) })}</span>
                  </div>
                  <Button
                    variant="ghost"
                    size="sm"
                  ...
                  >
                    {deletingId === passkey.credentialId ? "…" : t("delete")}
                  </Button>
                </li>
              ))}
            </ul>
          ) : (
            <p className="text-sm text-muted-foreground mb-4">{t("noPasskeys")}</p>
          )}

          {status === "success" && (
            <p className="text-sm text-green-600 mb-3">{t("passkeyAdded")}</p>
          )}
          {status === "error" && (
